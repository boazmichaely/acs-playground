...
(function () {
  ...
  const AUDIT_ADD_LINE_PHRASE_RE_GI =
    /(?:add\s+the\s+following\s+lines?\b|add\s+a\s+line\s+of\s+the\s+following\s+form\b|add\s+the\s+rules\s+below\b)/gi;
  const AUDIT_ADD_LINE_PHRASE_RE_I =
    /(?:add\s+the\s+following\s+lines?\b|add\s+a\s+line\s+of\s+the\s+following\s+form\b|add\s+the\s+rules\s+below\b)/i;
